...
final class SlaveIntColumnUpdater {
  int deleteCount = 0;
  ...
  SlaveIntColumnUpdater(String relvarName, IntColumn column, MasterBinaryTableUpdater master, ValueStoreUpdater store1, ValueStoreUpdater store2) {
  ...
  boolean contains12(int arg1, int arg2) {
    ...
    if (insertCount > 32) {
      if (!insertsSorted) {
        LongWithLongSorter.sort(insertSurrs, insertCount, insertValues);
        insertsSorted = true;
      }
    ...
    if (updateCount > 32) {
      if (!updatesSorted) {
        LongWithLongSorter.sort(updateSurrs, updateCount, updateValues);
        updatesSorted = true;
      }
  ...
  public void checkKey_12() {
    if (updateCount > 0) {
      ...
      for (int i=1 ; i < updateCount ; i++) {
        ...
        if (packedArgs == prevPackedArgs) {
          long value = updateValues[i];
          long prevValue = updateValues[i - 1];
          if (value != prevValue)
    ...
    if (insertCount > 0) {
      ...
      for (int i=0 ; i < insertCount ; i++) {
        ...
        if (i > 0 && packedArgs == prevPackedArgs) {
          long value = insertValues[i];
          long prevValue = insertValues[i - 1];
          if (value != prevValue)
            throw cols12KeyViolationException(arg1(packedArgs), arg2(packedArgs), value, prevValue, true);
        }
        ...
        if (assocSurr != 0xFFFFFFFF && column.contains1(assocSurr) && !wasDeleted(assocSurr) && !wasUpdated(packedArgs)) {
          long newValue = insertValues[i];
          long currValue = column.lookup(assocSurr);
    ...
    if (insertCount > 0 & updateCount > 0) {
      for (int i=0 ; i < insertCount ; i++) {
        ...
        if (idx != -1) {
          long insertValue = insertValues[i];
          long updateValue = updateValues[idx];
  ...
  private KeyViolationException cols12KeyViolationException(int arg1, int arg2, long value3, long otherValue3, boolean betweenNew) {
    Obj value1 = masterUpdater.store1.surrToValue(arg1);
    Obj value2 = masterUpdater.store2.surrToValue(arg2);
    Obj[] tuple1 = new Obj[] {value1, value2, IntObj.get(value3)};
    Obj[] tuple2 = new Obj[] {value1, value2, IntObj.get(otherValue3)};
