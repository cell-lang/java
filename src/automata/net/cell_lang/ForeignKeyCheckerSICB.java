...
class ForeignKeyCheckerSICB {
  SlaveIntColumnUpdater source;
  ...
  public ForeignKeyCheckerSICB(SlaveIntColumnUpdater source, MasterBinaryTableUpdater target) {
  ...
  private void checkSourceInsertions() {
    ...
    for (int i=0 ; i < count ; i++) {
      ...
      int arg1 = SlaveIntColumnUpdater.arg1(packedArgs);
      int arg2 = SlaveIntColumnUpdater.arg2(packedArgs);
  ...
  private void checkTargetClear() {
    ...
    IntColumn.Iter it = source.column.getIter();
    while (!it.done()) {
  ...
  private ForeignKeyViolationException insertionForeignKeyViolation(int surr1, int surr2, long value3) {
    Obj obj1 = source.store1.surrToValue(surr1);
    Obj obj2 = source.store2.surrToValue(surr2);
    Obj[] tuple = new Obj[] {obj1, obj2, IntObj.get(value3)};
  ...
  private ForeignKeyViolationException deletionForeignKeyViolation(int surr1, int surr2) {
    ...
    Obj obj3 = IntObj.get(source.column.lookup(assoc_surr));
