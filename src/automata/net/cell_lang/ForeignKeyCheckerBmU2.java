...
class ForeignKeyCheckerBmU2 implements UnaryTableUpdater.DeleteChecker {
  MasterBinaryTableUpdater source;
  ...
  public ForeignKeyCheckerBmU2(MasterBinaryTableUpdater source, UnaryTableUpdater target) {
    this.source = source;
    this.target = target;
  }
    ...
    if (source.hasInsertions()) {
      ...
      for (int i=0 ; i < count ; i++) {
        ...
        if (!target.contains(arg2))
          throw insertionForeignKeyViolation(MasterBinaryTableUpdater.arg1(entry), arg2);
      }
    }
