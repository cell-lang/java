...
class ForeignKeyCheckerSOCB {
  SlaveObjColumnUpdater source;
  ...
  public ForeignKeyCheckerSOCB(SlaveObjColumnUpdater source, MasterBinaryTableUpdater target) {
  ...
  private void checkSourceInsertions() {
    ...
    for (int i=0 ; i < count ; i++) {
      ...
      int arg1 = SlaveObjColumnUpdater.arg1(packedArgs);
      int arg2 = SlaveObjColumnUpdater.arg2(packedArgs);
  ...
  private void checkTargetClear() {
    ...
    ObjColumn.Iter it = source.column.getIter();
    while (!it.done()) {
  ...
  private ForeignKeyViolationException insertionForeignKeyViolation(int surr1, int surr2, Obj obj3) {
  ...
  private ForeignKeyViolationException deletionForeignKeyViolation(int surr1, int surr2) {
    ...
    Obj obj3 = source.column.lookup(assoc_surr);
