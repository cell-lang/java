...
class Test_ForeignKey_UT2 {
      ...
      for (int r1=1 ; r1 < r ; r1++)
        for (int r2=1 ; r2 < r ; r2++)
          for (int r3=1 ; r3 < r ; r3++)
            if (!done[r1][r2][r3]) {
              new Test_ForeignKey_UT2().run(r1, r2, r3, 10000);
              done[r1][r2][r3] = true;
            }
  }
  ...
  void run(int r1, int r2, int r3, int testCount) {
    ...
    targetUpdater = new UnaryTableUpdater(target, storeUpdater2);
    ...
    for (int _t=0 ; _t < testCount ; _t++) {
      ...
      for (int i=0 ; i < rand.nextInt(20) ; i++) {
        int idx = rand.nextInt(r2);
        int surr = storeUpdater2.lookupOrInsertValue(Conversions.stringToObj(values2[idx]));
        if (targetBitmap[idx])
...
loop1:    for (int i1=0 ; i1 < r1 ; i1++)
      ...
      boolean ok2 = targetUpdater.checkForeignKeys_2(sourceUpdater);
...
//           }
        ...
        boolean ok3 = targetUpdater.checkForeignKeys_2(sourceUpdater);
        boolean ok4 = targetUpdater.checkForeignKeys_2(sourceUpdater);
        ...
        for (int i1=0 ; i1 < r1 ; i1++)
          for (int i2=0 ; i2 < r2 ; i2++)
            for (int i3=0 ; i3 < r3 ; i3++) {
              ...
              if (surr2 == -1) {
                // If the second argument is not in the corresponding value store,
                // then neither table can contain it
                check(!inSource & !inTarget);
                continue;
              }

              // Since the second argument is in the value store, it has to appear in
              // either the second column of the source relation, or in the target one.
              // Since presence in the latter implies presence in the former, the
              // source relation must always contain it.
              check(source.contains2(surr2));

              if (surr1 == -1 | surr3 == -1) {
                // If either the first or the third argument is not in the corresponding value store,
