      ...
      for (int r1=1 ; r1 < r ; r1++)
        for (int r2=1 ; r2 < r ; r2++)
          for (int r3=1 ; r3 < r ; r3++)
            new Test_ForeignKey_TU2().run(r1, r2, r3, 10000);
    ...
    for (int _t=0 ; _t < testCount ; _t++) {
      ...
      for (int i1=0 ; i1 < r1 ; i1++)
        for (int i2=0 ; i2 < r2 ; i2++)
          ...
          if (newSourceBitmap[i1][i2][i3] & !newTargetBitmap[i2]) {
            ok = false;
            break;
          }

      boolean debugIt = sourceUpdater.checkForeignKeys_2(targetUpdater) != ok;

      if (debugIt) {
        System.out.printf("ERROR: _t = %d, r1 = %d, r2 = %d, r3 = %d, ok = %s, okCount = %d, notOkCount = %d\n",
          _t, r1, r2, r3, ok ? "true" : "false", okCount, notOkCount);

        ok = sourceUpdater.checkForeignKeys_2(targetUpdater);
        ...
        for (int i1=0 ; i1 < r1 ; i1++)
          for (int i2=0 ; i2 < r2 ; i2++)
            for (int i3=0 ; i3 < r3 ; i3++) {
              ...
              check(target.contains(surr2) & inTarget);
